    ...
    $.elastic_grid = {
        ...
        for (itemIdx = 0; itemIdx < c; itemIdx++)
            if (void 0 != a.items[itemIdx]) {
                ...
                strTag = strTag.substring(1), liObject.attr("data-tags", strTag), aObject = $("<a></a>"), aObject.attr("href", "javascript:;;"), imgObject = $("<img/>"), imgObject.attr({"src": f.thumbnail[0],"align" : "center"}), imgObject.attr("data-largesrc", f.large[0]), imgObject.attr("alt", f.title), imgObject.attr("title", f.title), spanObject = $("<span></span>"), spanObject.html(f.title), figureObject = $("<figure></figure>"), figureObject.append(spanObject), imgObject.appendTo(aObject), figureObject.appendTo(aObject), aObject.appendTo(liObject), liObject.appendTo(e)
            }
                    ...
                    if (eldata = a.items[o], this.$title.html(eldata.title), this.$description.html(eldata.description), this.$detailButtonList.html(""), urlList = eldata.button_list, urlList.length > 0)
                        for (g = 0; g < urlList.length; g++) {
                            var d = urlList[g].new_window ? "_blank" : "_self",
                                e = $('<a target="' + d + '"></a>');
                            e.addClass("link-button"), 0 == g && e.addClass("first"), e.attr({"href": urlList[g].url,"align" : "center"}), e.html(urlList[g].title), this.$detailButtonList.append(e)
                        }
                    var h = this;
                    if ("undefined" != typeof h.$largeImg && h.$largeImg.remove(), glarge = eldata.large, gthumbs = eldata.thumbnail, glarge.length == gthumbs.length && glarge.length > 0) {
                        var i = $("<ul></ul>");
                        for (g = 0; g < gthumbs.length; g++) {
                            var j = $("<li></li>"),
                                e = $('<a href="javascript:;;"></a>'),
                                k = $("<img/>");
                            k.addClass("related_photo"), 0 == g && k.addClass("selected"), k.attr({
								"src": gthumbs[g],
								"align" : "center",
								"data-large": glarge[g],
								"alt": f.title,
								"title": f.title
								}), e.append(k), j.append(e), i.append(j)
                        }
                        i.addClass("elastislide-list"), i.elastislide();
                        var l = $('<div class="elastislide-wrapper elastislide-horizontal"></div>');
                        l.append(i).find(".related_photo").bind("click", function () {
                            l.find(".selected").removeClass("selected"), $(this).addClass("selected"), $largePhoto = $(this).data("large"), $("<img/>").load(function () {
                                h.$fullimage.find("img").fadeOut(500, function () {
                                    $(this).fadeIn(500).attr({
										"src": $largePhoto,
										"align" : "center",
										"alt": f.title,
										"title": f.title
                        ...
                        }), h.$details.append('<div class="infosep"></div>'), h.$details.append(l)
                    } else h.$details.find(".infosep, .og-grid-small").remove();
                    h.$fullimage.is(":visible") && (this.$loading.show(), $("<img/>").load(function () {
                        var a = $(this);
                        a.attr("src") === h.$item.children("a").find("img").data("largesrc") && (h.$loading.hide(), h.$fullimage.find("img").remove(), h.$largeImg = a.fadeIn(350), h.$fullimage.append(h.$largeImg))
                    }).attr({
						"src": eldata.large[0],
						"align" : "center"
						}))
